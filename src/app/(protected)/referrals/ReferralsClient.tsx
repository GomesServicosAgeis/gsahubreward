...
export default function ReferralsClient({
  ...
  return (
    <main className="max-w-7xl mx-auto p-6 lg:p-10 min-h-screen bg-transparent text-[#E2E8F0]">
      <h1 className="text-4xl font-extrabold mb-4 bg-gradient-to-r from-[#7C3AED] to-[#C084FC] bg-clip-text text-transparent">
        Programa de Indicações
      </h1>
      <p className="text-xl text-[#94A3B8] mb-10 italic">
        Indique amigos e ganhe créditos nominais. Chegue a até <strong className="text-[#28A745]">80% de desconto permanente</strong>.
      </p>

      {/* Nível atual e progresso - Estilo Dashboard */}
      <div className="bg-[#1E293B]/70 backdrop-blur-md p-8 rounded-2xl border border-[#334155] mb-12 shadow-2xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div>
            <h2 className="text-sm font-black text-[#94A3B8] uppercase tracking-[0.2em]">Seu nível atual</h2>
            <div className="flex items-center gap-4 mt-3">
              <span className="text-5xl drop-shadow-[0_0_15px_rgba(124,58,237,0.3)]">{currentLevel.icon}</span>
              <span className="text-4xl font-black bg-gradient-to-r from-[#A78BFA] to-[#C084FC] bg-clip-text text-transparent uppercase tracking-tighter">
                {currentLevel.name}
              </span>
            </div>
          </div>
          <div className="bg-[#0a0015]/50 p-6 rounded-2xl border border-[#334155] text-center md:text-right min-w-[200px]">
            <p className="text-5xl font-black text-[#28A745] italic">{activeIndications}</p>
            <p className="text-[#94A3B8] font-bold uppercase text-[10px] tracking-[0.2em] mt-1 text-center">indicações ativas</p>
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-end px-1">
              <p className="text-[#94A3B8] text-sm">
                Faltam <strong className="text-white font-black">{nextLevel.min - activeIndications}</strong> para <strong className="text-[#C084FC]">{nextLevel.name} {nextLevel.icon}</strong>
              </p>
      ...
      <section className="mb-16">
        <h2 className="text-3xl font-bold mb-8 bg-gradient-to-r from-[#A78BFA] to-[#C084FC] bg-clip-text text-transparent flex items-center gap-3">
          <Trophy size={28} className="text-[#C084FC]" /> Links de Indicação
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {referralCodes.length > 0 ? (
                ...
                >
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <span className={`text-[10px] uppercase font-black px-3 py-1 rounded-md border ${code.is_unlocked ? 'border-green-500/30 text-green-400 bg-green-500/10' : 'border-red-500/30 text-red-400 bg-red-500/10'}`}>
                      {code.is_unlocked ? 'Liberado' : 'Pendente'}
                    </span>
                  </div>

                  <div className="bg-[#0a0015]/60 p-4 rounded-xl mb-6 font-mono text-xs break-all text-[#A78BFA] border border-[#334155]/50">
                    {link}
                  </div>
                  ...
                  <div className="flex justify-center group-hover:scale-105 transition-transform duration-500">
                    <div className="bg-white p-4 rounded-2xl shadow-[0_0_40px_rgba(255,255,255,0.08)]">
            ...
            <div className="col-span-full text-center py-20 bg-[#1E293B]/40 rounded-3xl border border-dashed border-[#334155]">
              ...
              <Link href="/dashboard" className="px-8 py-3 bg-[#334155] hover:bg-[#475569] text-white rounded-xl font-bold transition-all">
                Voltar ao Dashboard
              </Link>
            </div>
          )}
      ...
      {/* Tabela de Indicados Estilizada */}
      <section className="pb-20">
        ...
        <div className="bg-[#1E293B]/70 backdrop-blur-md rounded-2xl border border-[#334155] overflow-hidden shadow-2xl">
          <table className="w-full text-left">
            <thead className="bg-[#0a0015]/50 border-b border-[#334155] text-[#94A3B8] uppercase text-[10px] font-black tracking-widest">
              <tr>
                <th className="px-8 py-5">Usuário / E-mail</th>
                <th className="px-8 py-5">Produto GSA</th>
                <th className="px-8 py-5">Status Rewards</th>
              ...
              {referralsList.map((ref: any) => (
                <tr key={ref.id} className="hover:bg-[#7C3AED]/5 transition-colors group">
                  <td className="px-8 py-5">
                    <div>
                      <p className="font-black text-[#E2E8F0] uppercase tracking-tighter italic">{ref.referred_tenant_name}</p>
                      <p className="text-[11px] text-[#64748B]">{ref.referred_tenant_email}</p>
                    </div>
                  </td>
                  <td className="px-8 py-5 font-bold text-[#A78BFA] text-xs uppercase">{ref.product_name}</td>
                  <td className="px-8 py-5">
